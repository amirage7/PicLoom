...
- 图片节点名称显示与编辑（项目内归一化唯一）、选择、复制、删除与多来源版本关系持久化
- 版本连线支持多来源（多图 `@` 合成时全部引用图连线到结果图），选中连线后可按 Delete 删除关系且不影响两端图片
- 生成任务只导入 ChatGPT 最新回复中的最终结果图，回复中的候选稿与用户附件不会重复入库
- 桌面版图片详情面板提供“移除背景”一键操作：自动生成引用当前图片的固定 Prompt 并提交 ChatGPT 生成透明背景版本
- ChatGPT Prompt 区提供“透明背景”开关，无需参考图即可直接要求生成无背景新图片，也可与多张 `@图片` 配合
- 生成图优先采用同一次 ChatGPT 回复给出的简短名称；本地兜底名称会清除 `@`，且不额外发送命名消息
- 右侧面板查看/保存原图、默认适应窗口、编辑名称、Prompt 和标签、复制 Prompt
- 项目创建、重命名、删除和切换
...
安装包输出到 `desktop/release-new/AI Image Canvas-Setup-0.2.0.exe`。安装版会自动启动仅监听 `127.0.0.1:8001` 的本地后端。
